...
public class testECBwithUI extends JFrame {

    private JButton encryptButton;
    private JButton decryptButton;

    private ECBmode ecbEncryptor = new ECBmode();
    private ECBmode ecbDecryptor = new ECBmode();

    public testECBwithUI() {
        initUI();
    }
    ...
    private void performEncryption() {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ecbEncryptor.addKey(new int[]{10, 12, 13, 14});

                // Read the image file and perform encryption
                performFileOperation(selectedFile, ecbEncryptor, "_encrypted");
    ...
    private void performDecryption() {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            File selectedFile = fileChooser.getSelectedFile();
            try {
                ecbDecryptor.addKey(new int[]{10, 12, 13, 14});

                // Read the encrypted image file and perform decryption
                performFileOperation(selectedFile, ecbDecryptor, "_decrypted");
    ...
    private void performFileOperation(File inputFile, ECBmode encryptor, String suffix) throws IOException {
        int[] img = new int[2];
        boolean check = true;
        ...
        try (FileInputStream imgIn = new FileInputStream(inputFile);
            ...
            while (dataIn.available() > 0) {
                try {
                    ...
                    check = true;
                    img[1] = dataIn.readInt();
                    int[] result = encryptor.encrypt(img);
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                testECBwithUI ex = new testECBwithUI();
                ex.setVisible(true);
            }
        });
    }
